 ...
 * 如为单体应用，则会将添加此注册的Controller 方法缓存在 MappingBeanUrlNotLoginMethodHandle 中，通过传入 URL判断是否为不需登录的接口(通过NotLoginAnnotationUrl类中提供的方法 isNotLogin(String url)进行验证)，具体看代码注释
 * 基于Swagger获取URL名称
 * @author HuangLongPu
 */
@Target({ METHOD })
